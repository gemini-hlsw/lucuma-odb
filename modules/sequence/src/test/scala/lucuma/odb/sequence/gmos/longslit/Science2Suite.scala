...
class Science2Suite extends ScalaCheckSuite {

  import Science2.Adjustment
  import Science2.Goal
  ...
  case class Case(goal: Int*):
    def expected(timeSpan: TimeSpan): List[Goal] =
      goals(goal.toList.fproductLeft { total =>
        val perBlock = (Science2.SciencePeriod.toMicroseconds / timeSpan.toMicroseconds).toInt
        perBlock min total
      })
