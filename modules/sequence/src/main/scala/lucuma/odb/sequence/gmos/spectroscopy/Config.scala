 ...
 * slit and MOS), from which a science sequence may be generated.
 *
 * Both modes are generated by the same code, so everything the generator reads
 * is declared here.  What varies is the aperture — a builtin FPU for long slit,
 * a custom mask for MOS — which is exposed as two members because the two uses
 * diverge: `fpuMask` is what the generated steps carry, while `gcalFpu` is the
 * builtin FPU the smart gcal tables are keyed on.  For long slit they describe
 * the same aperture; for MOS `gcalFpu` is the builtin long slit FPU equivalent
 * to the custom mask's slit width.
 *
 * The acquisition configuration is not here: long slit has one, MOS does not.
 *
