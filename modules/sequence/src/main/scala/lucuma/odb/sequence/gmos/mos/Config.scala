 ...
 * with no acquisition.  The science sequence is generated by the shared long
 * slit generator; only the aperture differs.
 *
...
sealed trait Config[G: Enumerated, L: Enumerated, U] extends SpectroscopyConfig[G, L, U]:
  ...
  /**
   * Whether the acquisition image is taken with the mask in or out the light
   * path.
   */
