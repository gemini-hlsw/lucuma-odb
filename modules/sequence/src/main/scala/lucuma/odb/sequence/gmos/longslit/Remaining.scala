...
sealed trait Remaining[A]:
  ...
  def decrementAll(m: Map[A, Int]): Remaining[A] =
    m.iterator.foldLeft(this) { case (r, (a, i)) =>
      (0 until i).foldLeft(r)((rʹ, _) => rʹ.decrement(a))
    }
...
object Remaining:
  ...
  private case class Counts(
    ...
    def completeOne: Counts =
      copy(completed = NonNegInt.unsafeFrom(completed.value + 1))
  ...
  private case class Impl[A](
    ...
    override def decrement(a: A): Remaining[A] =
      decrementImpl(a)

    private def decrementImpl(a: A): Impl[A] =
      val nOpt = elementMap.get(a)
      val mOpt = nOpt.map(_.completeOne)  // reducing remaining by 1

      val elementMapʹ   = mOpt.fold(elementMap)(elementMap.updated(a, _))
      val remainingMapʹ = nOpt.fold(remainingMap): n =>

        // Remove from the current count entry
        val as      = remainingMap(n.remaining).excl(a)
        val removed = if as.isEmpty then remainingMap.removed(n.remaining) else remainingMap.updated(n.remaining, as)
    ...
    private def take1: Option[(A, Impl[A])] =
      ...
      remainingMap.lastOption.filter(_._1.value > 0).flatMap: (_, as) => // selects the group with the most remaining
        as.minOption(using order).fproduct(decrementImpl)
