...
object Acquisition2:
  ...
  private sealed trait StepComputer[D, G, L, U] extends GmosSequenceState[D, G, L, U]:
    ...
    def compute(
      ...
      def lastExpTime(exposureTime: AcqExposureTime): TimeSpan =
        Acquisition2.MaxExpTimeLastStep min (exposureTime * NonNegInt.unsafeFrom(3)).timeSpan
      ...
      eval {
        ...
        } yield Acquisition2.Steps(s0, s1, s2)
      }
  ...
  private sealed trait AcquisitionState[D] extends Acquisition2[D]:
  ...
  def gmosNorth(config: Config.GmosNorth, exposureTime: AcqExposureTime): Acquisition2[GmosNorth] =
  ...
  def gmosSouth(config: Config.GmosSouth, exposureTime: AcqExposureTime): Acquisition2[GmosSouth] =
    AcquisitionState.Init(
