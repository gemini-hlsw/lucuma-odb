    ...
    new ItcClient[IO] {
      ...
      override def spectroscopy(input: SpectroscopyInput, useCache: Boolean): IO[ClientCalculationResult] = {
        ...
        val wavelength = input.mode match
          case lucuma.itc.client.InstrumentMode.Flamingos2Spectroscopy(_, d, _, _)         => d.wavelength
          case lucuma.itc.client.InstrumentMode.GmosNorthSpectroscopy(_, w, _, _, _, _, _) => w
          case lucuma.itc.client.InstrumentMode.GmosSouthSpectroscopy(_, w, _, _, _, _, _) => w
