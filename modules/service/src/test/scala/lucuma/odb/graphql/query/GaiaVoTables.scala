...
/**
 * Canned Gaia responses for tests that must not reach the real catalog. Shared so that suites
 * outside the `GuideEnvironmentSuite` family can stub Gaia without duplicating the VOTable.
 */
object GaiaVoTables:

  /** Multiple candidate stars around the target of `createTargetWithProfileAs`, for AGS to choose from. */
  val multipleCandidates: String =
