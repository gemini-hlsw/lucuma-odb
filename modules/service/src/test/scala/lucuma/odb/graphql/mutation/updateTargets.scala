...
class updateTargets extends OdbSuite {
  ...
  test("update target in photometric not allowed"):
    for {
      pid <- createProgramAs(pi)
      tid <- createTargetAs(pi, pid)
      oid <- createObservationAs(pi, pid, tid)
      ...
      _   <- expect(
               ...
               """,
               expected = Left(List(s"Target $tid cannot be edited because it is in a calibration observation that does not allow target modifications."))
             )
    } yield ()

  test("update target in program is allowed"):
    for {
      pid <- withServices(service) { s =>
               Services.asSuperUser:
                 s.session.transaction.use { xa =>
