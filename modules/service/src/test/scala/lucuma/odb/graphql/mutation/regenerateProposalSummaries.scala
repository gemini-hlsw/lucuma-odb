...
class regenerateProposalSummaries extends OdbSuite
  ...
  test("submission enqueues one pending job per partner") {
    ...
    yield
      assertEquals(jobs.map(j => (j.partner, j.style, j.state)), List(
        (Partner.CA.some, SummaryStyle.GeminiStandard, "pending"),
  ...
  test("a claimed job is rendering and its payload can be built") {
    ...
    yield
      ...
      assertEquals(prepared.map(_.fileName.value.endsWith(s"-summary-${SummaryStyle.GeminiStandard.rendererName}.pdf")).head, true)
  ...
  test("regenerating while a job is waiting is a no-op") {
    ...
    yield
      ...
      assertEquals(after.map(_.style), List(SummaryStyle.GeminiStandard, SummaryStyle.NoirlabDarp))
  }
  ...
  test("a rendered job becomes a SUMMARY attachment for its partner, replacing the previous one, and is deleted") {
    ...
    yield
      ...
      assertEquals(first.map(s => (s.partner, s.fileName.endsWith(".pdf"), s.style)), List((Some("CA"), true, "GEMINI_STANDARD"), (Some("US"), true, "NOIRLAB_DARP")))
