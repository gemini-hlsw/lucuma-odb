...
class PdfSummaryJobDaemonSuite extends OdbSuite with ObservingModeSetupOperations:
  ...
  // Fake renderers: one that always writes `content`, one that always fails.
  def constant(content: String): PdfRenderer[IO] =
    (_, _, output) =>
      Stream.emit(content).through(fs2.text.utf8.encode).through(Files[IO].writeAll(output)).compile.drain.map(_.asRight)
