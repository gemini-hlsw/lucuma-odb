...
class PdfRendererSuite extends CatsEffectSuite:
  ...
  val itcUrl: Uri = Uri.unsafeFromString("http://itc.test/itc")
  ...
  def render(style: SummaryStyle, timeout: FiniteDuration = 10.seconds): IO[(Either[PdfRenderer.Error, Unit], Option[String])] =
    fixture.use: (script, out) =>
      for
        r <- PdfRenderer.subprocess[IO](script.toString, itcUrl, timeout).render(payload, style, out)
  ...
  test("a renderer that floods stdout still finishes"):
