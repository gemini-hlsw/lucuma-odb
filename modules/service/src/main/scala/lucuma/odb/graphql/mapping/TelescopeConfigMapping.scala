...
trait TelescopeConfigMapping[F[_]] extends EnumeratedOffsetTable[F] with StepRecordView[F]:
  ...
  private def enumeratedTelescopeConfigMapping: ObjectMapping =
    ObjectMapping(TelescopeConfigType)(
      SqlField("observationId", EnumeratedOffsetTable.ObservationId, key = true, hidden = true),
      SqlField("role",          EnumeratedOffsetTable.OffsetGeneratorRole, key = true, hidden = true),
      SqlField("index",         EnumeratedOffsetTable.Index, key = true, hidden = true),
      SqlObject("offset"),
      SqlField("guiding",       EnumeratedOffsetTable.GuideState)
    )
