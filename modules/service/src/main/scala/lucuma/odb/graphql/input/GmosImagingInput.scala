...
object GmosImagingInput:
  ...
  case class Create[F](
    filters: NonEmptyList[GmosImagingFilterInput.Create[F]],
  ...
  object Create:
    ...
    private def binding[F](
      FilterBinding: Matcher[GmosImagingFilterInput.Create[F]]
    ): Matcher[Create[F]] =
    ...
    val NorthBinding: Matcher[Create[GmosNorthFilter]] =
      binding(GmosImagingFilterInput.Create.NorthBinding)


    val SouthBinding: Matcher[Create[GmosSouthFilter]] =
      binding(GmosImagingFilterInput.Create.SouthBinding)
  ...
  case class Edit[F](
    filters: Option[NonEmptyList[GmosImagingFilterInput.Edit[F]]],
    common:  Edit.Common
  ):
    def toCreate: Result[Create[F]] =
      filters.fold(
        OdbError.InvalidArgument("At least one filter must be specified for GMOS imaging observations.".some).asFailure
      )(_.traverse(_.toCreate).map(fs => Create(fs, common.toCreate)))
  ...
  object Edit:
    ...
    private def binding[F](
      FilterBinding: Matcher[GmosImagingFilterInput.Edit[F]]
    ): Matcher[Edit[F]] =
      ObjectFieldsBinding.rmap:
        case List(
    ...
    val NorthBinding: Matcher[Edit[GmosNorthFilter]] =
      binding(GmosImagingFilterInput.Edit.NorthBinding)

    val SouthBinding: Matcher[Edit[GmosSouthFilter]] =
      binding(GmosImagingFilterInput.Edit.SouthBinding)
