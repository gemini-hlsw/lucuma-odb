  ...
  trait AsterismInput {
    ...
    def asterism: Nullable[NonEmptyList[Target.Id]] =
      Nullable.orAbsent(targetEnvironment).flatMap: t =>
        t match
          case TargetEnvironmentInput.Create(asterism = asterism) => Nullable.orAbsent(asterism.flatMap(NonEmptyList.fromList))
          case TargetEnvironmentInput.Edit(asterism = asterism)   =>
            asterism.flatMap(tids => Nullable.orAbsent(NonEmptyList.fromList(tids)))
    ...
    def explicitSignalToNoiseTargetId: Nullable[Target.Id] =
      Nullable.orAbsent(targetEnvironment).flatMap:
        case TargetEnvironmentInput.Create(explicitSignalToNoiseTargetId = sn) => Nullable.orAbsent(sn)
        case TargetEnvironmentInput.Edit(explicitSignalToNoiseTargetId = sn)   => sn
        ...
        case List(
          ...
          (rSubtitle,
            ...
            (rTimingWindows, rScheduling).parFlatMapN {
              ...
              case (Some(t), None)    => SchedulingConstraintsInput(None, None, Nullable.NonNull(t)).some.success
      ...
      ObjectFieldsBinding.rmap {
        case List(
          ...
          (rSubtitle,
            ...
            rConstraintSet,
            (rTimingWindows, rScheduling).parFlatMapN {
              case (Nullable.Null,       Nullable.Absent)     => Nullable.NonNull(SchedulingConstraintsInput(None, None, Nullable.Null)).success
              case (Nullable.Null,       Nullable.Null)       => Nullable.Null.success
              case (Nullable.Absent,     s)                   => s.success
              case (Nullable.NonNull(t), Nullable.Absent)     => Nullable.NonNull(SchedulingConstraintsInput(None, None, Nullable.NonNull(t))).success
