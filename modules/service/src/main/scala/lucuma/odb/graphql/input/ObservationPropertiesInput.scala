  ...
  trait AsterismInput {
    ...
    def asterism: Nullable[NonEmptyList[Target.Id]] =
      Nullable.orAbsent(targetEnvironment).flatMap: t =>
        t match
          case TargetEnvironmentInput.Create(_, asterism, _, _, _, _, _) => Nullable.orAbsent(asterism.flatMap(NonEmptyList.fromList))
          case TargetEnvironmentInput.Edit(_, asterism, _, _, _, _, _)   => asterism.flatMap(tids => Nullable.orAbsent(NonEmptyList.fromList(tids)))
    ...
    def explicitSignalToNoiseTargetId: Nullable[Target.Id] =
      Nullable.orAbsent(targetEnvironment).flatMap:
        case TargetEnvironmentInput.Create(_, _, sn, _, _, _, _) => Nullable.orAbsent(sn)
        case TargetEnvironmentInput.Edit(_, _, sn, _, _, _, _)   => sn
