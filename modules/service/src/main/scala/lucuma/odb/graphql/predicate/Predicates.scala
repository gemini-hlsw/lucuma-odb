...
trait Predicates[F[_]] extends BaseMapping[F]:
  ...
  object Predicates:
    ...
    val refreshArchiveDuplicationResult = RefreshArchiveDuplicationResultPredicates(Path.from(RefreshArchiveDuplicationResultType))
    ...
    val tooTrigger                      = TooTriggerPredicates(Path.from(TooTriggerType))
    val tooTriggerChronicleEntry        = TooTriggerChronicleEntryPredicates(Path.from(TooTriggerChronicleEntryType))
    val tooTriggerEdit                  = TooTriggerEditPredicates(Path.from(TooTriggerEditType))
