...
trait CallForProposalsView[F[_]] extends BaseMapping[F]:
  ...
  object CallForProposalsView extends TableDef("v_cfp"):
    ...
    object gemini:
      ...
      val Type               = col("c_gemini_proposal_type",         gemini_proposal_type)
      val Instruments        = col("c_gemini_instruments",           _instrument)
      val Proprietary        = col("c_gemini_proprietary",           int4_nonneg)
      val AllowsNonPartner   = col("c_gemini_allows_non_partner",    bool)
      val NonPartnerDeadline = col("c_gemini_non_partner_deadline",  core_timestamp.opt)
      val ExchangePartners   = col("c_gemini_exchange_partners",     _exchange_partner)
