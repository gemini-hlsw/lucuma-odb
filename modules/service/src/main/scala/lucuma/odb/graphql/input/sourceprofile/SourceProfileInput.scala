...
object SourceProfileInput {
  ...
  implicit class SourceProfileOps(self: SourceProfile) {
    def point:    Result[Point]    = self match { case a: Point    => Result(a); case _ => Result.failure(Matcher.validationProblem("Not a point source. To change profile type, please provide a full definition.")) }
    def uniform:  Result[Uniform]  = self match { case a: Uniform  => Result(a); case _ => Result.failure(Matcher.validationProblem("Not a uniform source. To change profile type, please provide a full definition.")) }
    def gaussian: Result[Gaussian] = self match { case a: Gaussian => Result(a); case _ => Result.failure(Matcher.validationProblem("Not a gaussian source.  To change profile type, please provide a full definition.")) }
  }
    ...
    ObjectFieldsBinding.rmap {
      case List(
        ...
        (rPoint, rUniform, rGaussian).parTupled.flatMap {
          ...
          case _                             => Result.failure(Matcher.validationProblem("Expected exactly one of point, uniform, or gaussian."))
        }
    }
