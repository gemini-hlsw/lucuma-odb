        ...
        (ctx.params match
          ...
          case GeneratorParams(_, _, config: f2.longslit.Config, _, _, _) =>
            EitherT.leftT[F, ExecutionDigest](
              Error.sequenceUnavailable(ctx.oid, s"Cannot calculate digest: F2 longslit not implemented")
            )
        )
        ...
        ctx.params match
          ...
          case GeneratorParams(_, _, config: f2.longslit.Config, _, _, _) =>
            EitherT.leftT[F, InstrumentExecutionConfig](
              Error.sequenceUnavailable(ctx.oid, s"F2 longslit not implemented.")
            )
