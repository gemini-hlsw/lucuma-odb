 ...
 * attachment, or records the failure.
 *
...
object PdfSummaryJobDaemon:
  ...
  val Channel = ident"ch_summary_job"
  ...
  def run[F[_]: {Async, Files, LoggerFactory as LF, Tracer as T}](
    ...
    // Debugging only: nothing cleans up kept files.
    ...
    def logged(what: String)(fa: F[Unit]): F[Unit] =
      fa.handleErrorWith(e => L.error(e)(s"PDF summary job daemon: $what failed"))
    ...
    def render(prepared: PdfSummaryJobService.Prepared): F[Unit] =
      workspace.use: dir =>
        ...
        renderer.render(prepared.payload, prepared.job.style, out).flatMap:
          case Left(err)  => err.asLeft.pure[F]
          case Right(()) => upload(prepared, out).as(().asRight)
      .handleError(e => PdfRenderer.Error(s"${e.getClass.getSimpleName}: ${e.getMessage}", permanent = false).asLeft)
      .flatMap:
    ...
    val daemon: F[Unit] =
      for
        ...
        // whole thing rather than sit there deaf until the next deploy.
        _    <- Stream(events, polls, drains).parJoinUnbounded
                  .attempts(Stream.constant(WaitToRestart))
                  .evalTap:
                    case Left(e)  => Logger[F].error(e)(s"PDF summary job daemon crashed, restarting in $WaitToRestart")
                    case Right(_) => ().pure[F]
                  .compile.drain
      yield ()
