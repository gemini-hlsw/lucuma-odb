...
object TargetService {
  ...
  def instantiate[F[_]: {Concurrent, Services, Tracer as T, Logger as L}]: TargetService[F] =
    new TargetService[F] {
      ...
      override def createTarget(
        ...
        input.foldWithId(OdbError.NotAuthorized(user.id).asFailureF): (input, pid) =>
          val insertFragment: ResultT[F, AppliedFragment] = input.subtypeInfo match
            ...
            case OpportunityInput.Create(r) =>
              Statements.insertOpportunityFragment(pid, input.name, r, input.sourceProfile.asJson, disposition, role).pure
      ...
      private def updateTargetsImpl(input: TargetPropertiesInput.Edit, which: AppliedFragment)(using Transaction[F], SuperUserAccess): F[UpdateTargetsResponse] =
        ...
        // Updates to the user-supplied ephemeris, if any.  An opportunity target does not track,
        // so this reaches only a target whose own tracking is nonsidereal.
  ...
  object Statements {
    ...
    /**
     * A Target of Opportunity: a placeholder carrying a region and no coordinates at all. Every
     * tracking column is left null, which is what the field constraints require of a target that
     * does not track -- and it does not track because there is nothing there yet to track.
     */
    ...
    def subtypeInfoUpdates(tracking: SiderealInput.Edit | NonsiderealInput.Edit | OpportunityInput.Edit): List[AppliedFragment] =
      ...
      // The column updates for a tracking edit, without the discriminator.
      ...
      // An opportunity target is a placeholder and carries no coordinates, so every tracking
      // column is cleared.  This arm always sets c_type = 'opportunity', and serves both edits
      // that do so: touching an existing opportunity target's region, where the columns are
      // already null and each assignment is a no-op, and converting a sidereal or nonsidereal
      // target into one, which must not leave the old coordinates behind -- they belong to no
      // tracking type now, and sidereal_or_all_columns_null rejects the row.
      val ClearTracking =
        NullOutSiderealFields ++ NullOutNonsiderealFields
      ...
      tracking match {
        ...
        case opp: OpportunityInput.Edit =>
          ...
          void"c_type = 'opportunity'" :: regionUpdates ++ ClearTracking
