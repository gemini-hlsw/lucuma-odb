...
object CallForProposalsService:
  ...
  case class CfpProperties(
    ...
    // Exchange (non-Gemini) calls have no `CallForProposalsType` and so place
    // no science-subtype constraint on a linked proposal.
    def validateSubtype(sub: ScienceSubtype): Result[Unit] =
      gemini.fold(Result.unit): gem =>
        CfpProperties.mismatchedCfp(cid, gem.callType, sub)
          .asFailure
    ...
    new CallForProposalsService[F]:
      ...
      def createCallForProposals(
        ...
        input.fold(OdbError.InvalidArgument().asFailureF): (SET, _) =>
          ...
          // Only Gemini CfPs keep their instruments in `t_cfp_instrument`. The
          // exchange observatories store theirs in array columns directly.
  ...
  object Statements:
    ...
    val cfp_properties: Codec[CfpProperties] =
      (cfp_id *: semester *: cfp_properties_gemini.opt).to[CfpProperties]
    ...
    private case class CreateCols(
      ...
      subaruType:  Option[SubaruProposalType]
    )
    ...
    val InsertCallForProposals: Query[Create, CallForProposals.Id] =
      sql"""
        ...
        SELECT
          ...
          $date,
          $date,
          COALESCE(${int4_nonneg.opt}, (SELECT c_proprietary FROM t_cfp_type WHERE c_type = ${gemini_proposal_type.opt})),
