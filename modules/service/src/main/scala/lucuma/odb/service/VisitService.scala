   ...
   def instantiate[F[_]: Concurrent: UUIDGen](using Services[F]): VisitService[F] =
     new VisitService[F] {
       ...
       private def insert(
         ...
         val checkUser: Either[NotAuthorized, Unit] =
           user.role.access match {
             case Staff | Admin | Service => ().asRight
             case _                       => /* NotAuthorized(user).asLeft */ ().asRight // for now
           }
           ...
           session
             ...
             .recover {
               case SqlState.ForeignKeyViolation(_) =>
                 InsertVisitResponse.ObservationNotFound(observationId, instrument).asLeft
             }

         (for {
           _ <- EitherT.fromEither(checkUser)
         ...
         } yield InsertVisitResponse.Success(v)).merge
       }
