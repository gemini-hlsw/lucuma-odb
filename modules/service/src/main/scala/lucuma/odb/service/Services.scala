...
import lucuma.odb.logic.TimeEstimateService2
...
trait Services[F[_]]:
  ...
  /** Construct a `TimeEstimateService`, given a `CommitHash` and an `ItcClient`.*/
    def timeEstimateService(commitHash: CommitHash, itcClient: ItcClient[F], ptc: TimeEstimateCalculatorImplementation.ForInstrumentMode): TimeEstimateService2[F]
...
object Services:
  ...
  def forUser[F[_]](u: User, e: Enums, m: Option[Mapping[F]])(s: Session[F])(
    ...
    new Services[F]:
      ...
      def timeEstimateService(commitHash: CommitHash, itcClient: ItcClient[F], ptc: TimeEstimateCalculatorImplementation.ForInstrumentMode) = TimeEstimateService2.instantiate(commitHash, itcClient, ptc)
  ...
  object Syntax:
    ...
    def timeEstimateService[F[_]](commitHash: CommitHash, itcClient: ItcClient[F], ptc: TimeEstimateCalculatorImplementation.ForInstrumentMode)(using Services[F]): TimeEstimateService2[F] = summon[Services[F]].timeEstimateService(commitHash, itcClient, ptc)
