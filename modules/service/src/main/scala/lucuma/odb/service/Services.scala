...
trait Services[F[_]]:
  ...
  /** The `GmosMosService`. */
  def gmosIfuService: GmosIfuService[F]
  ...
  /** The `SchedulingWindowService`. */
  def schedulingWindowService: SchedulingWindowService[F]
...
object Services:
  ...
  def forUser[F[_]: Tracer: Logger: LoggerFactory: UUIDGen: Async: Parallel](
    ...
    new Services[F]:
      ...
      lazy val gmosIfuService = GmosIfuService.instantiate
      ...
      lazy val schedulingWindowService = SchedulingWindowService.instantiate
  ...
  object Syntax:
    ...
    def gmosIfuService[F[_]](using Services[F]): GmosIfuService[F] = summon[Services[F]].gmosIfuService
    ...
    def schedulingWindowService[F[_]](using Services[F]): SchedulingWindowService[F] = summon[Services[F]].schedulingWindowService
