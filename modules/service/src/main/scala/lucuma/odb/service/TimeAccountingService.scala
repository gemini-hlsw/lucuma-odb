...
object TimeAccountingService {
        ...
        private val overlapDiscounts: StateT[F, TimeAccountingState, List[TimeCharge.DiscountEntry]] =
          StateT: tas =>
            (for
              ...
              o <- (v, r).traverseN((v, r) => session.option(SelectChargeableOverlap)(visitId, v.site, r)).map(_.flatten)
            yield o).map:
  ...
  object Statements {
    ...
    val SelectChargeableOverlap: Query[(Visit.Id, Site, TimestampInterval), (Timestamp, Observation.Id)] =
      sql"""
        SELECT
          e.c_received,
          e.c_observation_id
        FROM
          t_execution_event e
        WHERE
          e.c_visit_id  != $visit_id AND
          v.c_site       = $site     AND
          v.c_chargeable = true      AND
          e.c_received BETWEEN $core_timestamp AND $core_timestamp
        INNER JOIN t_visit v USING c_visit_id
        ORDER BY e.c_received
        LIMIT 1
      """.query(core_timestamp *: observation_id)
         .contramap { case (v, s, t) => (v, s, t.start, t.end) }
