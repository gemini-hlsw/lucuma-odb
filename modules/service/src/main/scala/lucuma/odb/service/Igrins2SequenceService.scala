...
trait Igrins2SequenceService[F[_]]:
  ...
  def selectDynamicForStep(
    stepId: Step.Id
  )(using Transaction[F]): F[Option[Igrins2DynamicConfig]]
...
object Igrins2SequenceService:
  ...
  def instantiate[F[_]: Concurrent](using Services[F]): Igrins2SequenceService[F] =
    ...
    new Igrins2SequenceService[F]:
      ...
      override def selectStatic(
        ...
        session.option(Statements.SelectStaticFromObservingMode)(observationId)
  ...
  object Statements:
    ...
    val SelectStaticFromObservingMode: Query[Observation.Id, Igrins2StaticConfig] =
      sql"""
        SELECT
          c_save_svc_images,
          c_offset_mode
    ...
    val SelectDynamicForStep: Query[Step.Id, Igrins2DynamicConfig] =
      sql"""
        SELECT
          #${encodeColumns(none, Igrins2DynamicColumns)}
        FROM t_igrins_2_dynamic
        WHERE c_step_id = $step_id
      """.query(igrins_2_dynamic)
