  ...
  object Statements:
    ...
    private def updatesForResult(r: Obscalc.Result): NonEmptyList[AppliedFragment] =
      NonEmptyList.of(
        ...
        sql"c_acq_offsets             = ${offset_array.opt}"(r.digest.map(_.acquisition.configs.toList.map(_.offset))),
        ...
        sql"c_sci_offsets             = ${offset_array.opt}"(r.digest.map(_.science.configs.toList.map(_.offset))),
