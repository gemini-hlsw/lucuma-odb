  ...
  object Statements:
    ...
    private def updatesForResult(
      ...
      NonEmptyList.of(
        ...
        sql"c_acq_gcal_set_count             = ${int4_nonneg.opt}"(r.digest.map(_.acquisition.gcalSets)),
        sql"c_acq_bias_count                 = ${int4_nonneg.opt}"(r.digest.map(_.acquisition.steps.biases.count)),
        sql"c_acq_bias_non_charged_time      = ${time_span.opt}"(r.digest.map(_.acquisition.steps.biases.time(ChargeClass.NonCharged))),
        sql"c_acq_bias_program_time          = ${time_span.opt}"(r.digest.map(_.acquisition.steps.biases.time(ChargeClass.Program))),
        sql"c_acq_dark_count                 = ${int4_nonneg.opt}"(r.digest.map(_.acquisition.steps.darks.count)),
        sql"c_acq_dark_non_charged_time      = ${time_span.opt}"(r.digest.map(_.acquisition.steps.darks.time(ChargeClass.NonCharged))),
        sql"c_acq_dark_program_time          = ${time_span.opt}"(r.digest.map(_.acquisition.steps.darks.time(ChargeClass.Program))),
        sql"c_acq_arc_count                  = ${int4_nonneg.opt}"(r.digest.map(_.acquisition.steps.arcs.count)),
        sql"c_acq_arc_non_charged_time       = ${time_span.opt}"(r.digest.map(_.acquisition.steps.arcs.time(ChargeClass.NonCharged))),
        sql"c_acq_arc_program_time           = ${time_span.opt}"(r.digest.map(_.acquisition.steps.arcs.time(ChargeClass.Program))),
        ...
        sql"c_sci_gcal_set_count             = ${int4_nonneg.opt}"(r.digest.map(_.science.gcalSets)),
        sql"c_sci_bias_count                 = ${int4_nonneg.opt}"(r.digest.map(_.science.steps.biases.count)),
        sql"c_sci_bias_non_charged_time      = ${time_span.opt}"(r.digest.map(_.science.steps.biases.time(ChargeClass.NonCharged))),
        sql"c_sci_bias_program_time          = ${time_span.opt}"(r.digest.map(_.science.steps.biases.time(ChargeClass.Program))),
        sql"c_sci_dark_count                 = ${int4_nonneg.opt}"(r.digest.map(_.science.steps.darks.count)),
        sql"c_sci_dark_non_charged_time      = ${time_span.opt}"(r.digest.map(_.science.steps.darks.time(ChargeClass.NonCharged))),
        sql"c_sci_dark_program_time          = ${time_span.opt}"(r.digest.map(_.science.steps.darks.time(ChargeClass.Program))),
        sql"c_sci_arc_count                  = ${int4_nonneg.opt}"(r.digest.map(_.science.steps.arcs.count)),
        sql"c_sci_arc_non_charged_time       = ${time_span.opt}"(r.digest.map(_.science.steps.arcs.time(ChargeClass.NonCharged))),
        sql"c_sci_arc_program_time           = ${time_span.opt}"(r.digest.map(_.science.steps.arcs.time(ChargeClass.Program))),
