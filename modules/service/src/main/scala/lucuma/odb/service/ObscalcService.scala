  ...
  object Statements:
    ...
    private def updatesForResult(r: Obscalc.Result): NonEmptyList[AppliedFragment] =
      NonEmptyList.of(
        ...
        sql"c_acq_offsets                = ${offset_array.opt}"(r.digest.map(_.acquisition.offsets.toList.map(_._2))),
        sql"c_acq_offset_guide_states    = ${_guide_state.opt}"(r.digest.map(_.acquisition.offsets.toList.map(_._1))),
        ...
        sql"c_sci_offsets                = ${offset_array.opt}"(r.digest.map(_.science.offsets.toList.map(_._2))),
        sql"c_sci_offset_guide_states    = ${_guide_state.opt}"(r.digest.map(_.science.offsets.toList.map(_._1))),
