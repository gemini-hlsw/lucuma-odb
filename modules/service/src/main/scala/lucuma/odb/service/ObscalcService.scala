  ...
  object Statements:
    ...
    private def obscalcResultColumns(prefix: Option[String]): String =
      prefixedColumns(
        ...
        "c_acq_atom_count",
        "c_acq_bias_count",
        "c_acq_bias_non_charged_time",
        "c_acq_bias_program_time",
        "c_acq_dark_count",
        "c_acq_dark_non_charged_time",
        "c_acq_dark_program_time",
        "c_acq_arc_count",
        ...
        "c_sci_atom_count",
        "c_sci_bias_count",
        "c_sci_bias_non_charged_time",
        "c_sci_bias_program_time",
        "c_sci_dark_count",
        "c_sci_dark_non_charged_time",
        "c_sci_dark_program_time",
        "c_sci_arc_count",
    ...
    private def updatesForResult(
      ...
      NonEmptyList.of(
        ...
        sql"c_acq_bias_count            = ${int4_nonneg.opt}"(r.digest.map(_.acquisition.biases.count)),
        sql"c_acq_bias_non_charged_time = ${time_span.opt}"(r.digest.map(_.acquisition.biases.time(ChargeClass.NonCharged))),
        sql"c_acq_bias_program_time     = ${time_span.opt}"(r.digest.map(_.acquisition.biases.time(ChargeClass.Program))),
        sql"c_acq_dark_count            = ${int4_nonneg.opt}"(r.digest.map(_.acquisition.darks.count)),
        sql"c_acq_dark_non_charged_time = ${time_span.opt}"(r.digest.map(_.acquisition.darks.time(ChargeClass.NonCharged))),
        sql"c_acq_dark_program_time     = ${time_span.opt}"(r.digest.map(_.acquisition.darks.time(ChargeClass.Program))),
        sql"c_acq_arc_count            = ${int4_nonneg.opt}"(r.digest.map(_.acquisition.arcs.count)),
        sql"c_acq_arc_non_charged_time = ${time_span.opt}"(r.digest.map(_.acquisition.arcs.time(ChargeClass.NonCharged))),
        sql"c_acq_arc_program_time     = ${time_span.opt}"(r.digest.map(_.acquisition.arcs.time(ChargeClass.Program))),
        sql"c_acq_flat_count            = ${int4_nonneg.opt}"(r.digest.map(_.acquisition.flats.count)),
        sql"c_acq_flat_non_charged_time = ${time_span.opt}"(r.digest.map(_.acquisition.flats.time(ChargeClass.NonCharged))),
        sql"c_acq_flat_program_time     = ${time_span.opt}"(r.digest.map(_.acquisition.flats.time(ChargeClass.Program))),
        sql"c_acq_observing_count            = ${int4_nonneg.opt}"(r.digest.map(_.acquisition.observing.count)),
        sql"c_acq_observing_non_charged_time = ${time_span.opt}"(r.digest.map(_.acquisition.observing.time(ChargeClass.NonCharged))),
        sql"c_acq_observing_program_time     = ${time_span.opt}"(r.digest.map(_.acquisition.observing.time(ChargeClass.Program))),
        ...
        sql"c_sci_bias_count            = ${int4_nonneg.opt}"(r.digest.map(_.science.biases.count)),
        sql"c_sci_bias_non_charged_time = ${time_span.opt}"(r.digest.map(_.science.biases.time(ChargeClass.NonCharged))),
        sql"c_sci_bias_program_time     = ${time_span.opt}"(r.digest.map(_.science.biases.time(ChargeClass.Program))),
        sql"c_sci_dark_count            = ${int4_nonneg.opt}"(r.digest.map(_.science.darks.count)),
        sql"c_sci_dark_non_charged_time = ${time_span.opt}"(r.digest.map(_.science.darks.time(ChargeClass.NonCharged))),
        sql"c_sci_dark_program_time     = ${time_span.opt}"(r.digest.map(_.science.darks.time(ChargeClass.Program))),
        sql"c_sci_arc_count            = ${int4_nonneg.opt}"(r.digest.map(_.science.arcs.count)),
        sql"c_sci_arc_non_charged_time = ${time_span.opt}"(r.digest.map(_.science.arcs.time(ChargeClass.NonCharged))),
        sql"c_sci_arc_program_time     = ${time_span.opt}"(r.digest.map(_.science.arcs.time(ChargeClass.Program))),
        sql"c_sci_flat_count            = ${int4_nonneg.opt}"(r.digest.map(_.science.flats.count)),
        sql"c_sci_flat_non_charged_time = ${time_span.opt}"(r.digest.map(_.science.flats.time(ChargeClass.NonCharged))),
        sql"c_sci_flat_program_time     = ${time_span.opt}"(r.digest.map(_.science.flats.time(ChargeClass.Program))),
        sql"c_sci_observing_count            = ${int4_nonneg.opt}"(r.digest.map(_.science.observing.count)),
        sql"c_sci_observing_non_charged_time = ${time_span.opt}"(r.digest.map(_.science.observing.time(ChargeClass.NonCharged))),
        sql"c_sci_observing_program_time     = ${time_span.opt}"(r.digest.map(_.science.observing.time(ChargeClass.Program))),
