-- Replace c_acq_read_mode (e_gnirs_obs_read_mode) with c_acq_type (e_gnirs_acquisition_type)
-- in t_gnirs_long_slit and its view.

-- New enum: classifies acquisition target brightness (VeryBright / Bright / Faint)
CREATE TYPE e_gnirs_acquisition_type AS ENUM ('VeryBright', 'Bright', 'Faint');

-- Drop the view first; it expands ls.* at creation time so it holds c_acq_read_mode implicitly.
DROP VIEW v_gnirs_long_slit;

-- Swap the column on the underlying table.
ALTER TABLE t_gnirs_long_slit DROP COLUMN c_acq_read_mode;
ALTER TABLE t_gnirs_long_slit
  ADD COLUMN c_acq_type e_gnirs_acquisition_type NOT NULL DEFAULT 'Faint';

-- Recreate v_gnirs_long_slit — body identical to V1146; ls.* now carries c_acq_type.
