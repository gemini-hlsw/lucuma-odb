...
UPDATE t_observation
   SET c_execution_requirement = CASE WHEN c_is_splittable THEN 'unconstrained'::e_execution_requirement
                                                           ELSE 'contiguous'::e_execution_requirement
                                 END;

-- Both views reference the column being dropped, so recreate them.  Neither
-- depends on the other (both select from t_observation directly), but v_observation
-- is `SELECT o.*` so it must be recreated after the column change regardless.
