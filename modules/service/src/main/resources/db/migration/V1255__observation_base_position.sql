-- Stored J2000 base position per observation, for the observations
-- targetCoordinates cone filter: the explicit base if set, otherwise the
-- asterism composite with every target proper-motion corrected to epoch
-- J2000.0.  Null when the observation contains a non-sidereal or opportunity
...
ALTER TABLE t_obscalc
  ADD COLUMN c_base_ra  d_angle_µas NULL,
  ADD COLUMN c_base_dec d_angle_µas NULL;

-- B-tree indexes to support fast cone (angular-distance) searches via
-- bounding-box prefiltering on the int8 microarcsecond columns, mirroring
-- the configuration-request indexes in V1250.
CREATE INDEX IF NOT EXISTS ix_obscalc_base_ra
  ON t_obscalc (c_base_ra);

CREATE INDEX IF NOT EXISTS ix_obscalc_base_dec
  ON t_obscalc (c_base_dec);
