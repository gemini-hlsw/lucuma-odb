...
ADD COLUMN c_explicit_spatial_offsets text NULL;

ALTER TABLE t_gmos_south_imaging
ADD COLUMN c_explicit_spatial_offsets text NULL;

ALTER TABLE t_gmos_north_imaging
ADD CONSTRAINT north_imaging_spatial_offsets_format CHECK (c_explicit_spatial_offsets IS NULL OR c_explicit_spatial_offsets ~ '^-?\d+(\.\d+)?(,-?\d+(\.\d+)?)*$');

ALTER TABLE t_gmos_south_imaging
ADD CONSTRAINT south_imaging_spatial_offsets_format CHECK (c_explicit_spatial_offsets IS NULL OR c_explicit_spatial_offsets ~ '^-?\d+(\.\d+)?(,-?\d+(\.\d+)?)*$');
