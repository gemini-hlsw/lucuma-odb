...
-- Remove any existing GMOS south imaging modes without a requirement ETM.
DELETE FROM t_gmos_south_imaging m
WHERE NOT EXISTS (
...
ALTER TABLE t_gmos_south_imaging_filter
  DROP CONSTRAINT t_gmos_south_imaging_filter_pkey;
...
FROM t_gmos_south_imaging_initial_filter;
...
DECLARE
  ...
  new_etm_id integer;
BEGIN
  FOR f IN SELECT * FROM t_gmos_south_imaging_filter LOOP
    SELECT *
      INTO req_etm
    ...
    IF FOUND THEN
      ...
      UPDATE t_gmos_south_imaging_filter
...
ALTER TABLE t_gmos_south_imaging_filter
  ...
  ADD CONSTRAINT t_gmos_south_imaging_filter_unique_exposure_time_mode_id UNIQUE (c_exposure_time_mode_id);

DROP VIEW v_gmos_south_imaging;
DROP VIEW v_gmos_south_imaging_filter;
DROP VIEW v_gmos_south_imaging_initial_filter;

-- No need for this now.
DROP TABLE t_gmos_south_imaging_initial_filter;
