...
ALTER TABLE t_program_user
  ADD COLUMN c_program_user_id d_program_user_id;
...
UPDATE t_program_user
   SET c_program_user_id = to_hex(seq.seq_val)
  FROM seq;

-- Default the id when new rows are inserted.
ALTER TABLE t_program_user
  ALTER COLUMN c_program_user_id SET DEFAULT to_hex(nextval('s_program_user_id'::regclass));
...
CREATE FUNCTION insert_invitation(
  ...
  BEGIN
    ...
    return p_program_user_id || '.' || invitation_key;
  END;
$$ LANGUAGE plpgsql;