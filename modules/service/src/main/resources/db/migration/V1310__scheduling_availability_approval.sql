-- Scheduling availability approval.
--
...
-- -- how long it is open, unrelated to how long it takes to execute.  The code
-- calls that total its scheduling *availability*, so that "window" keeps
-- meaning one entry in the list.  The requirement:
--
...
-- observation is covered by an approved configuration while its availability
-- is at least the recorded minimum; a shorter one -- windows where there were
-- none, or a smaller total -- needs a change request.  Widening is free.
--
-- TARGETS OF OPPORTUNITY
--
-- A ToO cannot state absolute dates, so it states a *ToO window* instead: a
-- length.  That stated length is its availability, and the trigger later opens
-- a real window of that length starting at the request.  Because the length is
-- what is recorded and compared, and it is never clipped by the end of the
-- semester, a 24 hour ToO triggered on the last night of the semester is still
-- a 24 hour ToO and stays covered.
--
-- Unstated, the ToO window defaults to 24 hours for RAPID and INTERRUPTING and
-- to unbounded for STANDARD, which is exactly what the database did before this
-- migration -- so nothing changes for a PI who says nothing.  There is no way
-- to state "unbounded" explicitly: it would differ from saying nothing only for
-- RAPID and INTERRUPTING, where it would suppress the automatic 24 hour window,
-- and a PI who wants that can state a length longer than the semester.
...
COMMENT ON COLUMN t_observation.c_too_window IS
  ...
  'triggered.  NULL means the PI stated nothing, in which case the activation '
  'supplies the default -- see too_window_effective().  There is deliberately '
  'no way to state "forever": a PI who wants an effectively unbounded window '
  'states a length longer than the semester.';

-- The ToO window in effect: the stated length, else the default for the
-- activation.  NULL means unbounded, which is the STANDARD default.
...
DECLARE
  len interval;
BEGIN
  SELECT too_window_effective(NEW.c_too_activation, o.c_too_window)
    INTO len
...
CREATE VIEW v_observation AS
  SELECT o.*,
  -- Grackle needs a nullable key for the nullable embedded TimeSpan.
  CASE WHEN o.c_too_window IS NOT NULL THEN o.c_observation_id END AS c_too_window_id,
...
-- Zero for rows that predate this migration: they were approved without an
-- availability being part of the bargain, and zero is subsumed by any
...
ALTER TABLE t_configuration_request
  ADD COLUMN c_min_availability interval NOT NULL DEFAULT INTERVAL '0 seconds'
    CHECK (c_min_availability >= INTERVAL '0 seconds');

COMMENT ON COLUMN t_configuration_request.c_min_availability IS
...
CREATE TRIGGER too_window_obscalc_invalidate_trigger
  AFTER UPDATE OF c_too_window ON t_observation
  FOR EACH ROW
  EXECUTE FUNCTION obsid_obscalc_invalidate();
...
COMMENT ON FUNCTION active_period_obscalc_invalidate() IS
  ...
  'period bounds the availability, so moving it can change which '
