-- The ITC models Altair from the guide star stored for the observation, which is
-- resolved against Gaia at the point of the remote call, so the generator needs
-- the stored name alongside the Altair configuration.
